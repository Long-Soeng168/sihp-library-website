...
import { BarChart3, CalendarDays, Monitor, Smartphone } from 'lucide-react';

const SummaryCards = () => {
    const { summary } = usePage<any>().props;

    const total = summary?.total_reads ?? 0;
    ...
    const stats = [
        ...
        {
            label: 'Desktop',
            value: summary?.desktop_reads ?? 0,
            percent: getPercentage(summary?.desktop_reads ?? 0),
        ...
        {
            label: 'Mobile',
            value: summary?.mobile_reads ?? 0,
            percent: getPercentage(summary?.mobile_reads ?? 0),
    ...
    return (
        <div className="grid grid-cols-1 gap-4 p-2 md:grid-cols-2 xl:grid-cols-4">
            {stats.map((stat, index) => (
                <div
                    key={index}
                    className="group relative overflow-hidden rounded-xl border bg-card p-5 transition-all duration-300 hover:border-primary/20 hover:shadow-md dark:hover:border-primary/40"
                >
