...
export interface CardTextOverlayBottomProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof cardVariants> {
...
const CardTextOverlayBottom = React.forwardRef<HTMLDivElement, CardTextOverlayBottomProps>(
        ...
        return (
            <div ref={ref} className={cn(cardVariants({ className }))} {...props}>
                ...
                <div className="relative z-10 flex h-full flex-col justify-end p-6 text-white transition-transform duration-500 ease-in-out group-hover:-translate-y-2">
                    <p className="text-sm font-medium tracking-wider text-gray-200 uppercase">- {category} -</p>
                    <h2 className="mt-1 text-3xl leading-tight font-bold tracking-tight text-white md:text-4xl">{title}</h2>
                </div>
            </div>
        );
    },
);
CardTextOverlayBottom.displayName = 'CardTextOverlayBottom';

export default CardTextOverlayBottom;
