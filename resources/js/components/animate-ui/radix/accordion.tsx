...
function AccordionItem({ className, children, ...props }: AccordionItemProps) {
    ...
    return (
        <AccordionItemContext.Provider value={{ isOpen, setIsOpen }}>
            <AccordionPrimitive.Item data-slot="accordion-item" className={cn('border-b border-border dark:border-white/50', className)} {...props}>
                {children}
...
function AccordionTrigger({
    ...
    return (
        <AccordionPrimitive.Trigger
            ...
            className={cn('flex flex-1 w-full items-center justify-between py-4 text-start font-medium hover:underline', className)}
            {...props}
        >
            {children}

            {chevron && (
                <motion.div data-slot="accordion-trigger-chevron" animate={{ rotate: isOpen ? 180 : 0 }} transition={transition}>
...
function AccordionContent({ className, children, transition = { type: 'spring', stiffness: 150, damping: 22 }, ...props }: AccordionContentProps) {
